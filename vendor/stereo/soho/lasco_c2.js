Stereo.soho.soho_lasco_c2 = function(){
	var handlers = {};
	...
	var _load = function (query){
		if(typeof(query) !== "object"){ Stereo.soho.warn("[LASCO/C2] _get:function - Invalid query."); return; }
		
		/* Validate query */
		var keys = ["num_img", "quality"];
		for(i in keys)
			if(typeof(query[keys[i]]) !== "string" && typeof(query[keys[i]]) !== "number"){ Stereo.soho.warn("[LASCO/C2] _get:function - Parameter \"num_img\" is string or number."); return; }

		$.get("http://sohodata.nascom.nasa.gov/cgi-bin/data_query_search_url?Session=web&Resolution="+query["quality"]+"&Display=Images&NumImg="+query["num_img"]+"&Types=instrument=LASCO:detector=C2").done(_parse);
	};
	...
	function _parse(response){
		if(!response || !response.results || response.results.length === 0){ Stereo.soho.warn("[LASCO/C2] _parse:function - Server no response."); return; }
		...
		function __done(){
			...
			if(typeof(vi) === "undefined"){
				...
				Stereo.soho.log("[LASCO/C2] _parse:__done:function - Image loaded: "+ $(this).attr("src"));
			} 
		}

		function __fail(){
			Stereo.soho.log("[LASCO/C2] _parse:__fail:function - Image unloaded: "+ $(this).attr("src"));
		}
	...
	this.attach = function(query){
		console.log("LASCO/C2");

		this.changeStatus("load");
		Stereo.soho.log("[LASCO/C2] attach:function - Query sent.");
