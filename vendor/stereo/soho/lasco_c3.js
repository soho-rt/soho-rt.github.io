Stereo.soho.soho_lasco_c3 = function(){
	var handlers = {};
	...
	var _load = function (query){
		if(typeof(query) !== "object"){ Stereo.soho.warn("[LASCO/C3] _get:function - Invalid query."); return; }
		
		/* Validate query */
		var keys = ["num_img", "quality"];
		for(i in keys)
			if(typeof(query[keys[i]]) !== "string" && typeof(query[keys[i]]) !== "number"){ Stereo.soho.warn("[LASCO/C3] _get:function - Parameter \"num_img\" is string or number."); return; }

		var url = encodeURIComponent("http://sohodata.nascom.nasa.gov/cgi-bin/data_query_search_url?Session=web&Resolution="+query["quality"]+"&Display=Images&NumImg="+query["num_img"]+"&Types=instrument%3DLASCO%3Adetector%3DC3");
	...
	function _parse(response){
		...
		function __done(){
			...
			if(typeof(vi) === "undefined"){
				...
				Stereo.soho.log("[LASCO/C3] _parse:__done:function - Image loaded: "+ $(this).attr("src"));
			} 
		}

		function __fail(){
			Stereo.soho.log("[LASCO/C3] _parse:__fail:function - Image unloaded: "+ $(this).attr("src"));
		}
	...
	this.attach = function(query){
		...
		Stereo.soho.log("[LASCO/C3] attach:function - Query sent.");
