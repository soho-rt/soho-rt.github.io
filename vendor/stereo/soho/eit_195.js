Stereo.soho.soho_eit_195 = function(){
	var handlers = {};
	...
	var _load = function (query){
		if(typeof(query) !== "object"){ Stereo.soho.warn("[EIT/195] _get:function - Invalid query."); return; }
		
		/* Validate query */
		var keys = ["num_img", "quality"];
		for(i in keys)
			if(typeof(query[keys[i]]) !== "string" && typeof(query[keys[i]]) !== "number"){ Stereo.soho.warn("[EIT/195] _get:function - Parameter \"num_img\" is string or number."); return; }
		
		$.get("http://sohodata.nascom.nasa.gov/cgi-bin/data_query_search_url?Session=web&Resolution="+query["quality"]+"&Display=Images&NumImg="+query["num_img"]+"&Types=instrument=EIT:wavelength=195").done(_parse);
	};
	...
	function _parse(response){
		if(!response || !response.results || response.results.length === 0){ Stereo.soho.warn("[EIT/195] _parse:function - Server no response."); return; }
		...
		function __done(){
			...
			if(typeof(vi) === "undefined"){
				...
				Stereo.soho.log("[EIT/195] _parse:__done:function - Image loaded: "+ $(this).attr("src"));
			} 
		}

		function __fail(){
			Stereo.soho.log("[EIT/195] _parse:__fail:function - Image unloaded: "+ $(this).attr("src"));
		}
	...
	this.attach = function(query){
		...
		Stereo.soho.log("[EIT/195] attach:function - Query sent.");
