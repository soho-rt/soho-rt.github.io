Stereo.soho.soho_hmi_magnetogram = function(){
	var handlers = {};
	...
	this.type = "soho_hmi_magnetogram";
	...
	var _load = function (query){
		if(typeof(query) !== "object"){ Stereo.soho.warn("[HMI/Magnetogram] _get:function - Invalid query."); return; }
		
		/* Validate query */
		var keys = ["num_img", "quality"];
		for(i in keys)
			if(typeof(query[keys[i]]) !== "string" && typeof(query[keys[i]]) !== "number"){ Stereo.soho.warn("[HMI/Magnetogram] _get:function - Parameter \"num_img\" is string or number."); return; }

		var url = encodeURIComponent("http://sohodata.nascom.nasa.gov/cgi-bin/data_query_search_url?Session=web&Resolution="+query["quality"]+"&Display=Images&NumImg="+query["num_img"]+"&Types=instrument=HMI:obs_type=Magnetogram");
	...
	function _parse(response){
		...
		function __done(){
			...
			if(typeof(vi) === "undefined"){
				...
				Stereo.soho.log("[HMI/Magnetogram] _parse:__done:function - Image loaded: "+ $(this).attr("src"));
			} 
		}

		function __fail(){
			Stereo.soho.log("[HMI/Magnetogram] _parse:__fail:function - Image unloaded: "+ $(this).attr("src"));
		}
	...
	this.attach = function(query){
		...
		Stereo.soho.log("[HMI/Magnetogram] attach:function - Query sent.");
