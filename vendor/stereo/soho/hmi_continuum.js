Stereo.soho.soho_hmi_continuum = function(){
	var handlers = {};
	...
	var _load = function (query){
		if(typeof(query) !== "object"){ Stereo.soho.warn("[HMI/Continuum] _get:function - Invalid query."); return; }
	...
	function _parse(response){
		if(!response || !response.results || response.results.length === 0){ Stereo.soho.warn("[HMI/Continuum] _parse:function - Server no response."); return; }
		...
		function __done(){
			...
			if(typeof(vi) === "undefined"){
				...
				Stereo.soho.log("[HMI/Continuum] _parse:__done:function - Image loaded: "+ $(this).attr("src"));
			} 
		}

		function __fail(){
			Stereo.soho.log("[HMI/Continuum] _parse:__fail:function - Image unloaded: "+ $(this).attr("src"));
		}
	...
	this.attach = function(query){
		...
		Stereo.soho.log("[HMI/Continuum] attach:function - Query sent.");
